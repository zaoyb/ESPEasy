...
#define PLUGIN_NAME_068       "Environment - KQM2801[TESTING]"
#define PLUGIN_VALUENAME1_068 "Tvoc"
...
void SHT3X::get()
{
  ...
  // I2C_write8_reg(
	...
	Wire.requestFrom(_i2c_device_address, (uint8_t)4);
	if (Wire.available() == 4)
	{
		data[0] = Wire.read();
		data[1] = Wire.read();
...
boolean Plugin_068(byte function, struct EventStruct *event, String& string)
{
	...
	switch (function)
	{
		...
		case PLUGIN_READ:
		{
			...
			sht3x->get();
			UserVar[event->BaseVarIndex + 0] = sht3x->tvoc;
			//UserVar[event->BaseVarIndex + 1] = sht3x->hum;
			String log = F("TVOC: tvoc: ");
